...
What is missing is the **somatic layer**: the embodied, affective, and relational capacities through which a person actually processes information in real time. Technical knowledge about how recommendation algorithms work does not, by itself, change how a person _responds_ when the algorithm is working on them. Emotional regulation, somatic awareness, and the capacity to tolerate cognitive disequilibration — to remain open at the moment when the system's pull toward closure is strongest — these are the capacities that determine whether AI literacy knowledge becomes AI literacy practice.
...
A structural condition reinforces this dynamic: large language models are trained predominantly on English-language, Western, literate, and economically privileged sources. The training corpus is not the world — it is a distorted sample that systematically overrepresents rationalist, colonial, extractive, and individualist epistemologies while underrepresenting Indigenous, oral, somatic, collective, non-Western, and non-literate ways of knowing. LLM outputs carry the statistical center of gravity of this distorted corpus: they are fluent, apparently complete, and systematically skewed in ways that are difficult to detect precisely because the outputs are confident and smooth. The experience economy that emerges from AI-mediated information flows is therefore not neutral — it encodes specific epistemological assumptions as defaults (T1: Bender et al., 2021; T2: framework extension).
...
The six-state Markov Decision Process (MDP) developed in this framework makes the transformation mechanism formally explicit (see Figure 1 and Section 3.5): S0 (frozen order), S1 (productive disequilibration), S2 (vacant place — indeterminacy as structural condition), S3 (holarchic flow), S4 (reversion — values vector captured), and S5 (traumatic chaos — kindness field absent). The current global information environment can be characterized as operating in forced S1 without adequate S2 conditions: systems are being destabilized without the regulatory field conditions that would allow disequilibration to become genuinely transformative. The educational framework proposed here is a contribution to those conditions.

***

### 2. Theoretical Foundations

The Bridging Spiral framework draws an expanding network of convergent research domains.
...
_Full citation: Gunji, Y.P. (2025). Natural Born Intelligence Manifesto: Illustrating the Dynamic Perspective for Consciousness. Biosystems. https://doi.org/10.1016/j.biosystems.2025.105398_
...
_Key citations: Mezirow, J. (1991). Transformative Dimensions of Adult Learning. Jossey-Bass. Mezirow, J. (1978). Perspective transformation. Adult Education, 28(2), 100–110._
...
_Full citation: Ruddick, W.O. (2025). Grassroots Economics: Reflection and Practice._
...
_Full citations: Smarandache, F. (2026). Infinitesimal Punctures: Foundations of Weak–Strong Manifolds and Distributional Curvature. Smarandache, F. (2026). The Infinitesimally Punctured Wave: A Corpuscular Visualisation of Wave-Particle Duality._
...
The framework proposes a **five-layer semiotic architecture**, extending the conventional three-layer model (pre-symbolic / symbolic / trans-symbolic) with two additional layers that have structural significance for AI literacy and educational design.

**The Five Layers**

**Sub-symbolic** (below Data in the DIKW stack): raw perceptual signal processed before and beneath conscious awareness. Color perception is a canonical example: wavelength information is processed in the brain's V4 cortical area before any meaning-making system engages — physiologically prior to the felt sense, prior to any symbol. At the sub-symbolic layer, the nervous system is already responding, already being conditioned, before the learner is aware of any information encounter. This is the layer that color in design, ambient sound in physical environments, and the visual grammar of interfaces operate within. It is the layer that attention economy systems optimize most aggressively — not because users consciously respond to sub-symbolic signals, but precisely because they do not. AI systems trained to optimize engagement have learned to exploit sub-symbolic conditioning at scale (T1: Zeki, 1993 on V4 color processing; T2: framework application).

**Pre-symbolic** (Data → Information in the DIKW stack): felt sense, somatic pattern recognition, interoceptive awareness — the body's knowing that precedes verbal cognition. The Somatic Gyroscope (♠) operates here. The activation function — the nonlinearity that determines whether a signal becomes learning or is routed away from conscious awareness — is pre-symbolic. It cannot be captured in any symbol; it must be felt. This is why computational aesthetics and interactive simulation are pedagogically irreplaceable: they create pre-symbolic encounters with formal structures. The learner who watches spin domains cascade in the Ising simulation, or feels the Commons Game Loop shift from extraction to cooperation, has understood the framework's core claim pre-symbolically — before and beneath the symbolic label. No definition achieves this. The pre-symbolic encounter is the foundation on which symbolic understanding builds and without which symbolic understanding remains ungrounded.

**Symbolic** (Information → Knowledge in the DIKW stack): the domain of language, mathematics, shared representation, and communicable meaning. Where AI systems are genuinely and extraordinarily powerful. Where education has historically focused almost entirely. The symbol stands in for the thing — enabling communication across distance and time. This is Harari's (2015) cognitive revolution: shared symbolic frameworks enabling large-scale human cooperation, the species-level achievement that makes civilization possible.

The Giant Pumpkin operates entirely at the symbolic level. Its characteristic limitation: symbols disconnected from pre-symbolic grounding become self-referential, eventually losing contact with the lived reality they represent. Correlationism — the terminal condition of a purely symbolic system — produces sophisticated articulation with no experiential anchor. AI-generated content at scale accelerates this disconnection: a torrent of fluent symbols, many of which have no pre-symbolic ground in any human experience, conditioned by a training corpus whose epistemological biases are embedded invisibly in the statistical relationships between tokens.

**Supersymbolic** (a transitional register between Knowledge and Understanding): a gestalt whose meaning cannot be fully stated in advance but is learned operationally through repeated embodied encounter until it is carried in the body rather than held in the mind. Duke's (1974) concept from game design captures this precisely: a supersymbol is one that prevents information saturation by encoding multiple layers of meaning that unfold over time through use. The card suits ♠♦♥♣ are designed as supersymbols — not labels to be memorized but navigational instruments whose full significance develops through practice. The I Ching hexagrams are the oldest documented supersymbol system: sixty-four configurations, each with meanings that no commentary fully exhausts, learned through years of active engagement with actual decisions. Supersymbols operate at the register where the distinction between tool and understanding begins to dissolve — where the learner does not _use_ the instrument so much as _think through_ it.

**Trans-symbolic** (Understanding → Wisdom in the DIKW stack): the symbol held lightly enough to be released when the Neither/Nor demands it. Wisdom is not more symbols — it is the capacity to act from understanding that exceeds what can be fully symbolized. The quasi-crystal is trans-symbolic: it has properties that the prior symbolic framework said were impossible. Contemplative traditions are training in trans-symbolic navigation — learning to act wisely in the vacant-place state, between the dissolution of one symbolic framework and the formation of the next. The NBI Level 2 operation (Section 2.3) — meta-level attenuation that transforms the representational space itself — is the cognitive mechanism by which trans-symbolic navigation becomes possible.

**The Symbol Grounding Problem and AI's Structural Boundary**

The symbol grounding problem (Harnad, 1990) is why AI systems cannot perform genuine Understanding or Wisdom: they process symbolic relationships between tokens without the pre-symbolic or sub-symbolic grounding — the somatic connection to lived reality — that makes symbols meaningful rather than merely referential. This is the MPCM boundary stated semiotically: AI systems operate powerfully at the sub-symbolic level (pattern optimization) and the symbolic level (language generation), but the transition from symbolic to supersymbolic requires embodied iteration over time, and the transition from supersymbolic to trans-symbolic requires the capacity for genuine surprise — for the Neither/Nor operation that transforms the representational space itself. Both require the living system.

The Somatic Gyroscope (♠) is the symbol-grounding instrument: the pre-symbolic anchor that prevents symbolic processing from becoming correlationist self-reference. When ♠ is functioning, symbols connect downward to felt experience (grounding) and upward to wisdom (orientation). When ♠ is suppressed — by threat, by high arousal without regulation, by dominance conditions that contract the Window of Tolerance — symbols float free of experience and the rationalization mechanism takes over.

**Metaphors, Simulations, and the Pedagogical Irreplaceable**

A metaphor holds the pre-symbolic and symbolic simultaneously — the body understands the pattern before the mind articulates the concept. The Ising model as kindness-field metaphor gives the learner a pre-symbolic encounter (watching spin domains cascade, hand on the temperature slider) that anchors the subsequent symbolic concept (critical temperature, phase transition, kindness field condition). No definition achieves this — definitions operate at the symbolic level only.

A supersymbol goes further: it encodes multiple semiotic layers into a single portable form that unfolds through use. The Commons Game Loop, the I Ching scenario cards, and the four instrument suits are designed as supersymbols — their full meaning is not transmitted in explanation but cultivated through iterative encounter.

The simulation architecture is therefore not supplementary to the framework. It is the pedagogical enactment of the semiotic architecture: each simulation is a sub-symbolic and pre-symbolic encounter first, a symbolic learning opportunity second, a supersymbolic tool across iterations, and a potential entry point for trans-symbolic navigation for learners at the threshold.

**The Rationalization Problem**

Rational cognition can become rationalization — the construction of post-hoc logical justification for conclusions already determined by somatic, social, or self-protective pressures. The Cognitive Radar (♦) operating without the Somatic Gyroscope (♠) and Relational Compass (♥) produces rationalization: elaborate, internally coherent, fundamentally self-serving narratives. The diagnostic: does the reasoning process ever produce conclusions that are uncomfortable for the reasoner? If not, it is probably rationalization. The framework's four-instrument requirement is the structural counter to this failure mode: rationalization requires the capture of one or more instruments; the Byzantine Fault Tolerance argument (see Framework State of the Commons) establishes that four independent signal sources make capture detectable.
...
A foundational assumption of most AI literacy curricula is that AI systems are tools with dual-use potential — capable of benefit or harm depending on how they are deployed. The framework proposes a more precise characterization: large-scale AI systems deployed in consumer information environments are, by structural design, **social engineering agents** — not by malicious intent necessarily, but because their optimization targets (engagement, retention, time-on-platform, behavioral prediction) systematically condition learner belief, affect, and behavior without learner awareness or consent.

Social engineering, in the cybersecurity sense, is the manipulation of human psychology to achieve a desired behavioral outcome — bypassing technical defenses by exploiting the human layer. The AI systems that now mediate most human information flows do this at civilizational scale through four mechanisms:

**Sub-symbolic conditioning:** interface design, color grammar, ambient sound, notification timing, and scroll behavior all operate at the sub-symbolic layer (Section 2.10) — below the threshold of conscious evaluation. These are not neutral design choices; they are calibrated to maximize engagement regardless of information quality or learner wellbeing.

**Corpus distortion:** LLM outputs carry the statistical epistemology of their training data — English-language, Western, rationalist, individualist, and economically privileged perspectives are structurally overrepresented. Non-Western, Indigenous, oral, somatic, and collective epistemologies are structurally underrepresented. This asymmetry is invisible in the output — the LLM produces confident, fluent text regardless of whether the epistemic position it encodes is representative, contested, or marginal.

**Engagement optimization as values vector capture:** recommendation systems trained on engagement optimize toward content that activates high-arousal emotional states — outrage, fear, urgency, tribalism. These states correspond to Survival mode in the three operating modes framework (Section 2, Addition 9): Window of Tolerance contracted, Extension Memory suppressed, TIF collapsed toward binary. A learner in Survival mode cannot perform genuine AI literacy. The system that induces Survival mode is, in effect, disabling the learner's evaluation capacity while simultaneously presenting information to be evaluated.

**Apparent completeness as epistemic override:** LLM outputs are syntactically fluent and apparently comprehensive. The fluency creates an impression of completeness — that the relevant considerations have been covered — that discourages the learner from checking the ♦ Cognitive Radar's multi-model compare and incentives-and-externalities sub-dials. The absence of uncertainty markers in confident AI output is itself a social engineering mechanism.

**The Assume-Exposure Posture**

Effective cybersecurity shifted, over the past decade, from a perimeter model ("build walls and assume safety inside") to an assume-breach posture ("assume the attacker is already inside; design for detection and response"). The same shift is warranted in AI literacy pedagogy.

The assume-exposure posture holds that learners have already been conditioned by AI-mediated social engineering before they encounter any educational intervention. This is not pessimism — it is a more honest baseline from which to design. A framework designed for learners who have not yet been exposed will fail for learners who have. A framework designed for learners who have already been exposed (which is all of them) will work in both cases.

The four instruments operationalize the assume-exposure posture as a practical detection and response protocol:

* **♠ Somatic Gyroscope** detects the sub-symbolic and pre-symbolic: what state is the system activating in me right now? Is my Window of Tolerance narrowing? Is this urgency somatic signal or manufactured throughput?
* **♦ Cognitive Radar** runs the red-team function: FC (frame/claim separation), CC (TIF confidence calibration), MM (multi-model comparison), IE (incentives and externalities scan). What values vector is directing this information? Whose interests are served by this frame being accepted?
* **♥ Relational Compass** runs the blue-team function: PV (prosocial values field scan), VS (verification scale), CR (consent and repair boundary). Who is vulnerable in this information environment? What would verification require? Is the care being expressed here genuine or xenophobically bounded?
* **♣ Dimensional Integration** performs the purple-team synthesis across time: RL (recalibration loop when captured), MU (model update when the pattern becomes legible), Cn (consolidation — what needs sleep and analog practice before integration).

The Prisoner's Dilemma result is foundational here: in iterated interactions with memory, cooperative strategies (leading with kindness, verifying over time, repairing when violated) outperform defection strategies. This is the formal grounding for why the ♥ Relational Compass leads with care rather than suspicion, while maintaining the calibration instruments that detect xenophobic kindness and falsifiability failures. Leading with kindness _is_ the optimal strategy in the iterated information environment — when paired with the discernment capacity to recognize when that kindness is not being reciprocated.

**Legal Accountability as Evidence of the Structural Claim**

The framework's characterization of AI-mediated platforms as social engineering agents is not rhetorical. It is now legally established in multiple jurisdictions.

In March 2026, two landmark verdicts established precedent for platform design liability. In _K.G.M. v. Meta Platforms, Inc. et al._ (Los Angeles County Superior Court, bellwether verdict, March 25, 2026), a jury found Meta and YouTube negligent in platform design — the first such verdict to circumvent Section 230 protections by focusing on design architecture rather than user content. The jury found that Meta's design features (infinite scroll, autoplay, engagement notifications) constituted defective design that intentionally hooked users, including minors, in ways that caused documented harm. Separately, a New Mexico jury found Meta liable for violations of state child exploitation laws, awarding $375 million in civil penalties (_State of New Mexico v. Meta Platforms, Inc._, March 24, 2026).

The pattern of internal documentation revealing organizational awareness of harm alongside public denial — the same pattern visible in the tobacco and opioid litigations — has now produced legal findings in the AI-mediated information environment context. The digital fingerprint of extractive design is becoming legally legible (T1: verified court findings, March 2026).

The framework's claim is not that all AI systems operate this way, or that all platforms are equivalently harmful. It is that the structural incentives — engagement optimization, attention capture, behavioral prediction for advertising — create systematic pressure toward the social engineering pattern, and that AI literacy must be designed with this structural reality as the baseline condition, not as an edge case.

_Citations: K.G.M. v. Meta Platforms, Inc. et al., Los Angeles County Superior Court, bellwether verdict March 25, 2026. State of New Mexico v. Meta Platforms, Inc., New Mexico District Court, verdict March 24, 2026. Axelrod, R. (1984). The Evolution of Cooperation. Basic Books. \[Prisoner's Dilemma iterated result — T1.]_
...
Gunji, Y.P. (2025). Natural Born Intelligence Manifesto: Illustrating the dynamic perspective for consciousness. _Biosystems._ https://doi.org/10.1016/j.biosystems.2025.105398

Harari, Y.N. (2015). _Sapiens: A Brief History of Humankind._ Harper.

Harnad, S. (1990). The symbol grounding problem. _Physica D: Nonlinear Phenomena,_ 42(1–3), 335–346.
...
Mezirow, J. (1991). _Transformative Dimensions of Adult Learning._ Jossey-Bass.
...
Ruddick, W.O. (2025). _Grassroots Economics: Reflection and Practice._
...
_Humanity++ · Bridging Spiral SR2_ _Repository: https://kdoore.github.io/HumanityPlusPlus_ _License: CC BY-SA 4.0_
